"""
Feature Engineering Module for Heatwave Prediction System (v2 — Improved)
Creates temporal features, lag features, rolling statistics, and heatwave-specific
compound features

Improvements over v1:
- Additional lag period (5-day) for better temporal coverage
- Rolling sum features (matching flood/landslide modules)
- Richer interaction features (8+ compound indicators)
- Heat-specific compound features (Urban Heat Island proxy, Heat Load Index)

References:
- IMD heatwave classification criteria
- NWS Heat Index formulation
- Perkins-Kirkpatrick & Lewis (2020) — Heatwave metrics review
"""
        ...
        for col in columns:
            if col in df.columns:
                for window in windows:
                    ...
                    # Rolling sum
                    df[f'{col}_rolling_sum_{window}d'] = df.groupby('city')[col].transform(
...
def create_interaction_features(df: pd.DataFrame) -> pd.DataFrame:
    ...
    Expanded set matching the richness of flood/landslide interaction features.
    """
    df = df.copy()
    
    # Temperature-Humidity interaction (heat stress indicator)
    ...
    # Temperature-Wind interaction (cooling effect — lower wind = trapped heat)
    ...
    # Radiation-Cloud interaction (solar heating potential)
    ...
    # Temperature-Soil moisture interaction (dry soil amplifies heat)
    ...
    # --- NEW interaction features (matching flood/landslide richness) ---
    
    # Temperature departure × consecutive hot days (persistent heatwave amplification)
    if all(col in df.columns for col in ['temp_departure', 'consecutive_hot_days']):
        df['departure_hotdays_interaction'] = df['temp_departure'] * df['consecutive_hot_days']
    
    # Heat index × consecutive dry days (drought amplifies heat stress)
    if all(col in df.columns for col in ['heat_index', 'consecutive_dry_days']):
        df['heat_drought_interaction'] = df['heat_index'] * df['consecutive_dry_days'] / 30
    
    # VPD × temperature (atmospheric drying capacity)
    if all(col in df.columns for col in ['vapour_pressure_deficit_max (kPa)', 'temperature_2m_max (°C)']):
        df['vpd_temp_interaction'] = df['vapour_pressure_deficit_max (kPa)'] * df['temperature_2m_max (°C)'] / 10
    
    # Nighttime × daytime temperature (warm nights = no recovery)
    if all(col in df.columns for col in ['temperature_2m_min (°C)', 'temperature_2m_max (°C)']):
        df['night_day_temp_interaction'] = df['temperature_2m_min (°C)'] * df['temperature_2m_max (°C)'] / 100
        df['diurnal_range_ratio'] = (df['temperature_2m_max (°C)'] - df['temperature_2m_min (°C)']) / (df['temperature_2m_max (°C)'] + 1)
    
    # Pressure × Temperature (high pressure systems trap heat)
    if all(col in df.columns for col in ['pressure_msl_mean (hPa)', 'temperature_2m_max (°C)']):
        df['pressure_temp_interaction'] = (df['pressure_msl_mean (hPa)'] - 1010) * df['temperature_2m_max (°C)'] / 100
    
    # Soil temperature × air temperature (urban heat island proxy)
    if all(col in df.columns for col in ['soil_temperature_0_to_7cm_mean (°C)', 'temperature_2m_mean (°C)']):
        df['soil_air_temp_coupling'] = df['soil_temperature_0_to_7cm_mean (°C)'] * df['temperature_2m_mean (°C)'] / 100
    
    # ET × Temperature (evaporative cooling deficit)
    if all(col in df.columns for col in ['et0_fao_evapotranspiration (mm)', 'temperature_2m_max (°C)']):
        df['et_temp_interaction'] = df['et0_fao_evapotranspiration (mm)'] * df['temperature_2m_max (°C)'] / 100
    
    # Heatwave Potential Index (composite — like flood_potential_index)
    components = []
    if 'temp_departure' in df.columns:
        components.append(df['temp_departure'].clip(lower=0) / (df['temp_departure'].max() + 1e-6))
    if 'vapour_pressure_deficit_max (kPa)' in df.columns:
        components.append(df['vapour_pressure_deficit_max (kPa)'] / (df['vapour_pressure_deficit_max (kPa)'].max() + 1e-6))
    if 'consecutive_hot_days' in df.columns:
        components.append(df['consecutive_hot_days'] / (df['consecutive_hot_days'].max() + 1e-6))
    if components:
        df['heatwave_potential_index'] = np.mean(components, axis=0)
