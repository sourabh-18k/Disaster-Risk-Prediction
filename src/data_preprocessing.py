...
def define_heatwave_labels(df: pd.DataFrame, city: str) -> pd.DataFrame:
    ...
    IMD Criteria:
    - Heatwave: Max temp ≥40°C (plains) and departure from normal ≥4.5-6.4°C
    - Severe Heatwave: Departure ≥6.5°C OR Max temp ≥47°C
    ...
    # Define thresholds based on city (plains vs coastal)
    if city in ['Bhubaneswar', 'Chennai']:  # Coastal
    ...
    df['is_hot_day'] = (df['temperature_2m_max (°C)'] >= base_temp_threshold) & \
                       (df['temp_departure'] >= 4.5)
    
    df['is_severe_hot_day'] = ((df['temp_departure'] >= 6.5) | \
                               (df['temperature_2m_max (°C)'] >= 47))
    ...
    df.loc[(df['temperature_2m_max (°C)'] >= base_temp_threshold - 2) & \
           (df['temp_departure'] >= 2) & \
           (df['temp_departure'] < 4.5), 'risk_level'] = 1
